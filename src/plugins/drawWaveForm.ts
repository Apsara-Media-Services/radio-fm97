...
export type DrawWaveformEvents = {
  startRecording: [];
  stopRecording: [];
};

class DrawWaveform extends BasePlugin<DrawWaveformEvents, DrawWaveformOptions> {
  private mediaRecorder: MediaRecorder | null = null;
  private recordedUrl = '';

  public static create(options?: DrawWaveformOptions) {
    return new DrawWaveform(options || {});
  }
  ...
  render(stream: MediaStream): () => void {
    ...
    const drawWaveform = () => {
      ...
      const color =
        this.options.realtimeWaveColor ||
